 ...
 */
/**
 * the master stream is a data stream that UEAuth can write transaction information to that exists outside the context of any one authgroup. For example, when a new Auth Group is created, it would be published to this stream so that other systems can react to the creation.
 * @export
 * @interface WriteEventStreamProviderMasterStream
 */
export interface WriteEventStreamProviderMasterStream {
    /**
     * 
     * @type {boolean}
     * @memberof WriteEventStreamProviderMasterStream
     */
    enabled?: boolean;
    /**
     * this is the topic or stream address to publish master stream content. For NATS use 'stream::subject'
     * @type {string}
     * @memberof WriteEventStreamProviderMasterStream
     */
    streamPath?: string;
