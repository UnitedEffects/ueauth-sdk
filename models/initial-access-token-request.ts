 ...
 * 
 * @export
 * @interface InitialAccessTokenRequest
 */
export interface InitialAccessTokenRequest {
    /**
     * This adds an email address to the token, locking its use to a user with that email address only. useful when providing a token as part of an invite; however, this is not required in all cases. For example, you do not need this to use an IAT for OIDC client creation.
     * @type {string}
     * @memberof InitialAccessTokenRequest
     */
    email?: string;
    /**
     * This adds a userId to the token, locking its use to a user with that id only.
     * @type {string}
     * @memberof InitialAccessTokenRequest
     */
    sub?: string;
    /**
     * Number of seconds after which this token expires. Default is 604800 (7 days).
     * @type {number}
     * @memberof InitialAccessTokenRequest
     */
    expires?: number;
